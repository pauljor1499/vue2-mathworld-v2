...
export class AssignmentService {
    ...
    delete_assignment_by_code(assignment_id) {
        var assignments = Classes[class_index].assignments;
        var assignment_index = assignments.findIndex(
            (test) => test.id === assignment_id
        );
            ...
            Classes.forEach((class_el) => {
                if (class_el.title == class_title) {
                    ...
                    if (index > -1) {
                        ...
                        class_el.assignments[index].date_open =
                            formatDate(date);
                    }
                }
            });
    ...
    get_assignment(class_title, assignment_code) {
        var class_index = Classes.findIndex(
            (class_el) => class_el.title == class_title
        );
        var assignments = Classes[class_index].assignments;
        var assignment_index = assignments.findIndex(
            (assignment) => assignment.id == assignment_code
        );
