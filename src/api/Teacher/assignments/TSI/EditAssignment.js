...
export class TSI_EditAssignment {
    set_assignment(class_title, assignment_id) {
        var index = Classes.findIndex(
            (class_el) => class_el.title == class_title
        );
        if (index > -1) {
            var assignment_index = Classes[index].assignments.findIndex(
                (assignment) => assignment.id == assignment_id
            );
            if (assignment_index > -1) {
                edit_assignment = JSON.parse(
                    JSON.stringify(Classes[index].assignments[assignment_index])
                );
            }
        }
    }
    ...
    submit_Step2(selected_questions) {
        try {
            edit_assignment.questions = JSON.parse(
    ...
    submit_Step3(start_date, end_date, class_assigned) {
        try {
            ...
            edit_assignment.assigned_classes = JSON.parse(
                JSON.stringify(class_assigned)
            );
            return true;
        } catch (err) {
            console.log(err);
        ...
        try {
            ...
            function remove_assignment_from_classes() {
                Classes.forEach((class_el) => {
                    var index = class_el.assignments.findIndex(
                        (assignment) =>
                            assignment.title == edit_assignment.title
                    );
            ...
            function add_assignment_to_classes(assigned_classes) {
                for (var i = 0; i < assigned_classes.length; i++) {
                    var index = Classes.findIndex(
                        (class_el) =>
                            class_el.title ==
                            edit_assignment.assigned_classes[i]
                    );
                    if (index > -1) {
