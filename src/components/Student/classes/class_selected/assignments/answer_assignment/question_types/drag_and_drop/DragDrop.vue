<template>
    <v-card-text class="question-body">
        ...
        <br />
        <v-card-actions>
            <v-btn text small color="error" @click="removeAnswer()">
                Remove answer
            </v-btn>
        </v-card-actions>
    </v-card-text>
    ...
    watch: {
        list2() {
            if (this.list2.length != 0) {
                ...
                AnswersService.prototype.remove_answer(
                    "drag-and-drop",
    ...
    methods: {
        load_data() {
            ...
            for (var i = 0; i < this.list2.length; i++) {
                var index = this.list1.findIndex(
                    (answer) => answer == this.list2[i]
                );
        ...
        removeAnswer() {
            this.list1 = JSON.parse(
                JSON.stringify(this.current_question.choices)
            );
            this.list2 = [];
        },
    },
