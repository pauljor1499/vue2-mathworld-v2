<template>
    <v-dialog max-width="600" v-model="dialog" persistent>
        <template v-slot:activator="{ on, attrs }">
            <v-btn outlined color="primary" v-bind="attrs" v-on="on" @click="get_current_data()">
        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <v-card-actions class="justify-end">
                    ...
                    <v-btn width="100" color="primary" @click="update_class()"> Update </v-btn>
                </v-card-actions>
            </v-card-text>
        </v-card>
    </v-dialog>
...
export default {
    ...
    computed: {
        ...
        timeStartErrors() {
            ...
            if ((this.time_start > this.time_end || this.time_start == this.time_end) && this.time_end !== "") {
        ...
        timeEndErrors() {
            ...
            if ((this.time_end < this.time_start || this.time_start == this.time_end) && this.time_start !== "") {
    ...
    methods: {
        get_current_data() {
            ...
            this.schedules = JSON.parse(JSON.stringify(this.get_class.schedules));
        },
        ...
        update_class() {
            ...
            if (this.check_errors()) {
                ...
                const results = this.class_store.update_class(this.get_class.uuid, updated_data);
                if (results) {
                    this.error = "";
                    this.error_message = false;
                    this.close_dialog();
                    this.$emit("clickClassUpdate");
                } else {
