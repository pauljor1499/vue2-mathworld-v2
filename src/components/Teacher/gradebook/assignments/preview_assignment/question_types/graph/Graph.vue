<template>
    <v-card-text class="question-body">
                ...
                >
                    <v-card-text class="list-item">
                        ...
                        <div class="item-content">
                            <v-text-field
                                v-model="item.latex"
                                @focus="item_focus(index)"
                                hide-details
                            />
                        </div>
                        <div class="item-actions">
                            <v-btn
                                @click="remove_expression(item.id)"
                                v-if="index != focusItem"
                                icon
                                small
                            >
                            ...
                            <v-btn
                                @click="edit_expression(index, item.latex)"
                                v-if="index == focusItem"
                                icon
                                small
                            >
                ...
                <v-card outlined hover>
                    <v-card-text class="list-item">
                        ...
                        <div class="item-content">
                            <v-text-field
                                v-model="inputValue"
                                @focus="item_focus(expressions.length + 1)"
                                @blur="focusItem = null"
                                hide-details
                            />
                        </div>
        ...
        <br />
        <v-card-actions>
            <v-btn text small color="error" @click="remove_answers()">
                Remove Answer
            </v-btn>
        </v-card-actions>
    </v-card-text>
...
export default {
    ...
    methods: {
        ...
        remove_expression(expression_id) {
            var index = this.expressions.findIndex(
                (item) => item.id == expression_id
            );
        ...
        set_answer() {
            AnswersService.prototype.set_answer(
                "graph-type",
