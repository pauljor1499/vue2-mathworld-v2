<template>
    <div class="answer-assignment" v-if="get_assignment !== ''">
        <YesNoOption
            v-if="yes_no_option.status"
            @clickNo="yes_no_option.status = false"
            @clickYes="clickYes()"
            :dialog_prop="this.yes_no_option"
        />
        <YesNoWarning
        ...
        <v-dialog v-model="dialog" width="600px" persistent>
            <v-card>
                ...
                <br />
                <v-card-text>
                    This is only being done for testing and won't affect any
                    information.
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
                ...
                <v-card>
                    <v-card-title>
                        <div class="question-title">
                            <span>
                                <v-icon dense left color="#FDCF2E">
                                    mdi-star
                                </v-icon>
                                {{
                                    get_assignment.questions[current_question]
                                        .points + " Points "
                                }}
                            </span>
                    ...
                    <MultipleChoice
                        v-if="
                            get_assignment.questions[current_question].type ==
                            'multiple-choice'
                        "
                        :current_question="
                            get_assignment.questions[current_question]
                ...
                <div class="question-actions">
                    ...
                    <v-btn
                        v-if="
                            ...
                            this.get_assignment.questions.length - 1
                        "
                        color="primary"
                        width="150"
                        ...
                        >
                            <div
                                :class="{
                                    'question-card-gray':
                                        question.flag === false &&
                                        my_answers[index].status == false,
                                    'question-card-ribbon':
                                        question.flag === true,
                                    'question-card-blue':
                                        question.flag === false &&
                                        my_answers[index].status == true,
                                    'question-card-hover':
                                        index == item_hover ||
...
import YesNoOption from "@/components/Student/main/dialogs/YesNoOption.vue";
import YesNoWarning from "@/components/Student/main/dialogs/YesNoWarning.vue";
...
export default {
    ...
    methods: {
        ...
        show_dialog() {
            if (
                this.get_total_answers !== this.get_assignment.questions.length
            ) {
                this.yes_no_warning.status = true;
                var total_no_answers =
                    this.get_assignment.questions.length -
                    this.get_total_answers;

                if (total_no_answers > 1) {
                    this.yes_no_warning.message =
                        "There are " +
                        total_no_answers +
                        " unanswered questions, do you want to submit all answers?";
                } else {
                    this.yes_no_warning.message =
                        "There is " +
                        total_no_answers +
                        " unanswered question, do you want to submit all answers?";
                }
            } else {
                this.yes_no_option.status = true;
                this.yes_no_option.message =
        ...
        get_question_flag() {
            this.current_flag =
                this.get_assignment.questions[this.current_question].flag;
        ...
        assignment_data() {
            var data = AssignmentService.prototype.get_assignment(
                this.$route.params.class_title,
                this.$route.params.assignment_code
            );

            this.get_assignment.id = data.id;
            this.get_assignment.title = data.title;
            this.get_assignment.description = data.description;
            this.get_assignment.calculator = false;
            this.get_assignment.questions = data.questions;
        },
    },
