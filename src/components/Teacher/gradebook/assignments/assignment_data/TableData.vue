                        ...
                        <tbody>
                            <tr v-for="(item, index) in items" :key="index">
                                ...
                                >
                                    <span
                                        style="color: green"
                                        v-if="check_answer(item2, index2)"
                                    >
                                        {{ item2 }}
                                    </span>
...
export default {
    ...
    created() {
        this.students = JSON.parse(
            JSON.stringify(
                AssignmentDataService.prototype.get_students(
                    this.$route.params.assignment_code
                )
            )
        );
        this.answers_headers = JSON.parse(
            JSON.stringify(
                AssignmentDataService.prototype.get_answers_headers(
                    this.$route.params.assignment_code
                )
            )
        );
        this.student_answers = JSON.parse(
            JSON.stringify(
                AssignmentDataService.prototype.get_student_answers(
                    this.$route.params.assignment_code
                )
            )
        );
        this.correct_answers = JSON.parse(
            JSON.stringify(
                AssignmentDataService.prototype.get_correct_answers(
                    this.$route.params.assignment_code
                )
            )
        );
