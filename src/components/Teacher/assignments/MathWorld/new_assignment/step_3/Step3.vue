...
export default {
    ...
    methods: {
        ...
        clickNext() {
            var data = {
                date_start: this.formatDate(this.range.start),
                date_end: this.formatDate(this.range.end),
                assigned_class: this.assigned_class,
            };
            this.$emit("checkStep3", data);
        },
        ...
        validateDate() {
            let today = new Date();
            if (this.range != null) {
                if (this.formatDate(this.range.start) < this.formatDate(today) && this.formatDate(this.range.end) < this.formatDate(today)) {
    ...
    computed: {
        class_names() {
            var results = [];
            this.items.forEach((class_el) => {
                results.push(class_el.title + " (" + class_el.section + ")");
            });
            return results.sort();
        },

        hasErrors() {
            if (this.range == null || this.startDateErrors.length != 0 || this.endDateErrors != 0 || this.assigned_class.length == 0) {
                return true;
            } else {
                return false;
            }
        ...
        startDateErrors() {
            ...
            !this.$v.range.start.required && errors.push("Start date is required");
        ...
        assignedClassErrors() {
            ...
            !this.$v.assigned_class.required && errors.push("Assigning a class is required");
            return errors;
        },
    },
    ...
    // created() {
    //     this.items.forEach((class_el) => {
    //         this.class_names.push(class_el.title + " (" + class_el.section + ")");
    //     });
    // },
};
</script>
