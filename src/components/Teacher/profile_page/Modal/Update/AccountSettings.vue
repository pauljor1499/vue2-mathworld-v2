...
export default {
    ...
    methods: {
        updateAccount() {
            if (this.account_password == this.current_password) {
                this.$emit("updateAccount", this.password);
                this.closeModal();
            } else {
    ...
    computed: {
        current_passwordErrors() {
            ...
            !this.$v.current_password.maxLength &&
                errors.push(
                    "Current password may not be longer than 25 characters"
                );
